...
import { CallParentFuntion, SocialAuthProps } from "../global/types";


const LogForm: Component<{ 
    onCallParentFunction?: CallParentFuntion,
    headerText?: string,
    privoders :  Component<SocialAuthProps>[]
    }> = (props) => {
    ...
    const returJsx = (component: Component<SocialAuthProps>) => {
        const ComponentMemo = createMemo(component(ButtonProps));
    ...
    <div class="mx-auto lg:px-8 max-w-screen-xl lg:py-16 grid lg:grid-cols-2 gap-2 lg:gap-8 place-items-center">
        <div class="w-full max-w-md min-w-max p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-6 md:p-8 dark:bg-gray-800 dark:border-gray-700">
            <h5 class="text-xl font-medium text-gray-900 dark:text-white"></h5>
            <ul class="my-4 space-y-3">
                <For each={props.privoders}>{(componentElement, i) =>
                    <li>
                        { returJsx(componentElement) }
                    </li>
                }</For>
            </ul>
            <form class="space-y-6 " action="#" onClick={handleChildClick}>
                <div>
