...
const SignInForm: Component<{}> = (props) => {
    ...
    const [erroMessage, setErroMessage] = createSignal("");
    ...
    return(
        <section class="bg-gray-200 dark:bg-gray-900  flex items-center justify-center w-screen h-screen">
            <div class="mx-auto lg:px-8 h-screen max-w-screen-xl lg:py-8 grid lg:grid-cols-3 gap-2 lg:gap-8 place-items-center">
                <div class="relative file:w-full max-w-sm p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-6 md:p-8 dark:bg-gray-800 dark:border-gray-700">
                    <Show when={errorLogIn()}> 
                        <div class="
                            ...
                            dark:text-red-400" 
                            role="alert">
                            <span class="font-medium">Something Whent Wrong!</span> {erroMessage()}
                        </div>
                    </Show>
                ...
                <img 
                    class="hidden col-span-2 lg:flex h-full w-full transition-all duration-300 rounded-lg cursor-pointer filter grayscale hover:grayscale-0 d" 
