...
interface succesInformation {
    username: string;
    email: string;
}

const SignUpForm: Component<{}> = (props) => {
    const [ signupSuccesInfo, setSignupSuccesInfo 
        ] = createSignal<succesInformation>({ username: "", email: ""});
    const [showSuccesPage, setShowSuccesPage] = createSignal(false);
    ...
    const getFormContent = async (event: SubmitEvent) => {
        ...
        if (form) {
            ...
            setAwaitLogin(false);
            if (data !== null && data.user.identities.length > 0) {
                const userInfo = {
                    username : data.user.user_metadata.username,
                    email: data.user.email
                }
                setSignupSuccesInfo(() => userInfo);
    ...
    return(
        ...
        <Show 
            ...
            fallback={
                <SignupSucces
                    username={signupSuccesInfo().username}
                    email={signupSuccesInfo().email}
                />
            }>
            <section class="bg-gray-200 dark:bg-gray-900  flex items-center justify-center w-screen h-screen">
                <div class="mx-auto lg:px-8 max-w-screen-xl lg:py-16 grid lg:grid-cols-2 gap-2 lg:gap-8 place-items-center">
                    <div class="relative first-line:w-fit max-w-md min-w-max p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-6 md:p-8 dark:bg-gray-800 dark:border-gray-700">
                    <Show when={errorLogIn()}> 
                        <div class="
                            ...
                            dark:text-red-400" 
                            role="alert">
                            <span class="font-medium">Something Whent Wrong!</span> User Account Already exist 
                        </div>
                    </Show>
